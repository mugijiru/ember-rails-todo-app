...
module("Unit | Controller | todo-items", function (hooks) {
  ...
  test("get savedTodoItems", async function (assert) {
    ...
    const newTodoItem = this.owner.lookup('service:store').createRecord('todo-item', { name: "new", isCompleted: false })
    const savedTodoItem = this.owner.lookup('service:store').createRecord('todo-item', { name: "saved", isCompleted: false })
  ...
  test("action build should set new todo-item from todoItems to editingTodoItem", async function (assert) {
    ...
    controller.todoItems = [...Array(3)].map((num) => store.createRecord('todo-item', { name: `item ${num}`, isCompleted: false }))
